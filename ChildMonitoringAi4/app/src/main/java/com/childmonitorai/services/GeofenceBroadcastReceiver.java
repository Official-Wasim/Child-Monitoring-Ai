...
public class GeofenceBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        // First check if geofence alerts are enabled
        Preferences preferences = new Preferences();
        if (!preferences.isGeofence()) {
            Log.d(TAG, "Geofence alerts are disabled in preferences");
            return;
        }
        ...
        if (!geofencingEvent.getTriggeringGeofences().isEmpty()) {
            ...
            GeofenceData geofenceData = FlaggedContents.getGeofenceById(geofenceId);
            String fenceName = geofenceData != null ? geofenceData.getName() : geofenceId;
            
            if (transitionType == Geofence.GEOFENCE_TRANSITION_ENTER) {
                title = "Geofence Alert";
                message = "Child has entered " + fenceName;
            } else if (transitionType == Geofence.GEOFENCE_TRANSITION_EXIT) {
                title = "Geofence Alert";
                message = "Child has left " + fenceName;
            } else {
                return;
            }
        } else {
